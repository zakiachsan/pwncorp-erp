  ...
  return (
    <div style={{ padding: "0 24px 24px" }}>
      ...
      <div style={{ background: "#fff", border: "1px solid #ecebea", borderRadius: 8, padding: 16 }}>
        <FInput label="PLAT NOMOR" placeholder="Contoh: B 1234 CD" />
        <FSelect label="MERK" options={["Toyota", "Honda", "Mitsubishi", "Suzuki", "Daihatsu", "Isuzu"]} />
        <FInput label="MODEL" placeholder="Contoh: Avanza" />
        <FInput label="TAHUN" placeholder="Contoh: 2022" />
        <FInput label="WARNA" placeholder="Contoh: Silver" />
        <FInput label="CUSTOMER" placeholder="Nama customer" />
        <div style={{ marginTop: 20 }}>
          <button onClick={() => { alert("Vehicle berhasil ditambahkan!"); router.push("/master-data/vehicles"); }} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "8px 20px", fontSize: 13, fontWeight: 600, color: "#fff", background: "#0176d3", border: "1px solid #0176d3", borderRadius: 6, cursor: "pointer" }}>
            <Save size={14} /> Simpan
          </button>
        </div>
      </div>

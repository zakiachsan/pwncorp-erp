  ...
  return (
    <div style={{ padding: "0 24px 24px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        ...
        <h1 style={{ fontSize: 18, fontWeight: 700, color: "#001526", margin: 0 }}>Add Supplier</h1>
      </div>
      <div style={{ background: "#fff", border: "1px solid #ecebea", borderRadius: 8, padding: 16 }}>
        <FInput label="NAMA" placeholder="Nama supplier" />
        <FInput label="TELEPON" placeholder="No. telepon" />
        <FInput label="ALAMAT" placeholder="Alamat lengkap" />
        <FSelect label="PAYMENT TERMS" options={["COD", "Net 7", "Net 14", "Net 30"]} />
        <div style={{ marginTop: 20 }}>
          <button onClick={() => { alert("Supplier berhasil ditambahkan!"); router.push("/master-data/suppliers"); }} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "8px 20px", fontSize: 13, fontWeight: 600, color: "#fff", background: "#0176d3", border: "1px solid #0176d3", borderRadius: 6, cursor: "pointer" }}>
            <Save size={14} /> Simpan
          </button>
        </div>
      </div>

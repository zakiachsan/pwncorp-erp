...
export default function SummaryPurchaseOrdersPage() {
  const [data, setData] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  useEffect(() => {
    fetch("/api/reports/purchase?report=summary-po&limit=100")
      .then((r) => r.json())
      .then((j) => {
        setData(j.data || []);
        setLoading(false);
  ...
  return (
    <div>
      <div className="view-header">
        ...
        <button onClick={() => exportTableToExcel(document.querySelector(".data-table"), makeFilename("summary-purchase-orders"))} className="btn btn--sm">
          ...
          <table className="data-table">
            ...
            <tbody>
              {data.map((row, i) => (
                <tr key={row.id}>
                  <td>{i + 1}</td>
                  <td className="font-medium text-[--color-brand-secondary]">{row.poNo || "—"}</td>
      ...
      {!loading && !error && (
        <div className="mt-3 text-sm text-[--color-text-secondary]">
          Showing {data.length} of {data.length} purchase orders
        </div>
      )}
    </div>
  );

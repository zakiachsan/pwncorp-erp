...
export default function ProductsPage() {
  ...
  const handleExport = () => {
    ...
    exportToCsv(exportData, sparepartColumns, makeFilename("products"));
  };
          ...
          <table className="data-table">
            ...
            <tbody>
              {products.map((product) => (
                <tr key={product.id} className="cursor-pointer" onClick={() => router.push(`/products/${product.sku}`)}>
                  <td>{product.sku}</td>
                  <td>{product.name}</td>
                  <td>{product.brand}</td>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "#fff", borderRadius: 12, padding: 24, maxWidth: 480, width: "90%", boxShadow: "0 8px 32px rgba(0,0,0,0.16)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ fontSize: 16, fontWeight: 600, color: "#001526" }}>Import Products</h3>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ padding: 16, background: "#f8f9fa", borderRadius: 8, border: "1px solid #e9ecef" }}>
                ...
                <button onClick={() => downloadTemplate(sparepartColumns, "products")} className="btn btn--sm" style={{ background: "#0176d3", color: "#fff" }}>

...
export default function PurchaseDeliveryDetailPage() {
  ...
  if (!delivery) {
    return (
      <div style={{ padding: 24 }}>
        <button onClick={() => router.push("/warehouse/purchase-deliveries")} style={S.backBtn}>
          <ArrowLeft size={16} /> Kembali
        </button>
        <div style={S.card}><p style={{ color: "#444746", fontSize: 14 }}>Purchase Delivery tidak ditemukan: {refCode}</p></div>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: "0 24px 24px" }}>
      {/* Back Button */}
      <button onClick={() => router.push("/warehouse/purchase-deliveries")} style={S.backBtn}>
        <ArrowLeft size={16} /> Purchase Delivery
      </button>

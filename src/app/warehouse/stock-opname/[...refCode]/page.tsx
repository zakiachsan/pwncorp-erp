...
export default function StockOpnameDetailPage() {
  ...
  const isCancelled = opname.status === "Cancelled";
  const currentStepIdx = workflowSteps.indexOf(opname.status?.toUpperCase());
  ...
  const confirmLabels: Record<string, { title: string; desc: string; color: string }> = {
    complete: { title: "Complete Opname?", desc: "Data opname dianggap benar.", color: "#0176d3" },
    approve: { title: "Approve Opname?", desc: "Stok akan disesuaikan dengan hasil hitung fisik. Tindakan ini tidak bisa dibatalkan.", color: "#2e844a" },
    cancel: { title: "Cancel Opname?", desc: "Opname akan dibatalkan.", color: "#ea001e" },
  };
    ...
    <div style={{ padding: "0 24px 24px" }}>
      ...
      <div className="view-header">
        ...
        <div className="flex gap-2">
          {isDraft && (
            <>
              <button onClick={() => setConfirmAction("complete")} className="btn btn--sm" style={{ background: "#0176d3", color: "#fff", border: "1px solid #0176d3" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, marginBottom: 20 }}>
        <div>
          ...
          <F label="STATUS" value={opname.status} />
        </div>
        <div style={{ borderLeft: "1px solid #ecebea", paddingLeft: 32 }}>
          ...
          <F label="CREATED AT" value={opname.createdAt ? new Date(opname.createdAt).toLocaleString("id-ID") : "-"} />
          <F label="TOTAL ADJUSTMENT" value={`${totalAdj > 0 ? "+" : ""}${totalAdj}`} />
        </div>
      </div>

      {/* Items Table */}
      <div style={{ fontSize: 13, fontWeight: 600, color: "#0176d3", marginBottom: 8 }}>Opname Items</div>
        ...
        <table style={S.table}>
          <thead>
            <tr>
              ...
              <th style={{ ...S.th, textAlign: "right" }}>System Qty</th>
              <th style={{ ...S.th, textAlign: "right" }}>Physical Qty</th>
              <th style={{ ...S.th, textAlign: "right" }}>Adjustment</th>
          ...
          <tbody>
            {items.map((item: any, idx: number) => (
              <tr key={item.id || idx} style={S.tr}>
                ...
                <td style={{ ...S.td, textAlign: "right" }}>{item.systemQty || 0}</td>
                <td style={{ ...S.td, textAlign: "right", fontWeight: 600 }}>{item.physicalQty || 0}</td>
                <td style={{ ...S.td, textAlign: "right", fontWeight: 600, color: (item.adjustment || 0) > 0 ? "#2e844a" : (item.adjustment || 0) < 0 ? "#ea001e" : "#001526" }}>
                  {(item.adjustment || 0) > 0 ? `+${item.adjustment}` : item.adjustment || 0}
                </td>

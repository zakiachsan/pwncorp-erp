...
export default function APChequePage() {
  const [rows, setRows] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  useEffect(() => {
    fetch("/api/reports/finance?report=ap-cheque")
      .then((r) => r.json())
      .then((j) => {
  ...
  return (
    <div>
      <div className="view-header">
        ...
        <button onClick={() => exportTableToExcel(document.querySelector(".data-table"), makeFilename("ap-cheque"))} className="btn btn--sm"><Download size={14} /> Export</button>
      </div>

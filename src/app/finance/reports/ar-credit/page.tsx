...
export default function ARCreditPage() {
  ...
  useEffect(() => {
    fetch("/api/reports/finance?report=ar-credit")
      .then((r) => r.json())
      .then((j) => {
  ...
  return (
    <div>
      <div className="view-header">
        ...
        <button onClick={() => exportTableToExcel(document.querySelector(".data-table"), makeFilename("ar-credit"))} className="btn btn--sm"><Download size={14} /> Export</button>
      </div>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="hover:bg-[#f8f8f8]">
                ...
                <td className="text-right">{fmt(r.amountPaid)}</td>
                <td className="text-right">{fmt(r.amountDue)}</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr><td colSpan={6} className="text-center py-8 text-[--color-text-secondary]">Tidak ada data</td></tr>
            )}
          </tbody>
        </table>
      </div>

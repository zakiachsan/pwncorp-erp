...
export default function AROverlimitPage() {
  const [rows, setRows] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  useEffect(() => {
    fetch("/api/reports/finance?report=ar-overlimit")
      .then((r) => r.json())
      .then((j) => {
    ...
    <div>
      <div className="view-header">
        <div className="view-title">
          ...
          Overlimit - Accounts Receivable
        </div>
        <button onClick={() => exportTableToExcel(document.querySelector(".data-table"), makeFilename("ar-overlimit"))} className="btn btn--sm"><Download size={14} /> Export</button>
      </div>
